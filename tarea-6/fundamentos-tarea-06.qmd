...
La gráfica de la distribución muestral de la media parece tender a una distribución normal. También la gráfica cuantil-cuantil pareciera que los cuantiles de la distribución muestral son similares a los de una distribución normal estandarizada. 
...
```{r}
prop_soles <- map_dbl(1:10000,  ~ mean(sample(muestra, 50)))
...
```{r}
ggplot(prop_soles_tbl, aes(x = prop_soles)) + geom_histogram(bins=40)
```
...
La distribución muestral de la proporción de soles parece comportarse como una distribución normal centrada en 0.78.
